...
export class UserService {
    ...
    async get(ID: string): Promise<User> {
        const user = await this.userModel.findOne({ id: ID }).select("-password -__v -_id").exec();
        if (!user) {
...
async create(createUserDto: CreateUserDto): Promise<User> {
        ...
        try {
            ...
            switch (e.code) {
                ...
                default:
                    throw new InternalServerErrorException("Internal server error: contact support: " + error_codes.default.UNABLE_TO_SAVE_VIOLATION);
            }
        }
    }
    ...
    async update(ID: string, updateUserDto: UpdateUserDto): Promise<User> {
        ...
        return this.userModel.findOneAndUpdate({ id: ID }, updateUserDto, { new: true });
    }
    ...
    async findByEmail(username: string): Promise<User> {
        const user = await this.userModel.findOne({ username }).select('-_id -__v').exec();
        if (!user) {
